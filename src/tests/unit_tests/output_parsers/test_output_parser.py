...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '{"key": "value"}',
            '{"key": "value"}',
        ),  # Test without any backslash to escape
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            '{"command": {"name": "foo", "args": {"arg1": "value1"}}}',
            TaskAction(name="foo", args={"arg1": "value1"}),
        ),
        (
            '{"command": {"name": "bar", "args": {"arg2": "value2"}}}',
            TaskAction(name="bar", args={"arg2": "value2"}),
        ),
        (
            '{"invalid_key": "value"}',
            TaskAction(
                name="ERROR",
                args={"error": "Incomplete command args: {'invalid_key': 'value'}"},
            ),
        ),
        (
            "not_a_json_string",
            TaskAction(
                name="ERROR",
                args={"error": "Could not parse invalid json: not_a_json_string"},
            ),
        ),
    ],
)
