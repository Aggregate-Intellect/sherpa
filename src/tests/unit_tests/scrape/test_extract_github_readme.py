...
def mock_request(url, **kwargs):
    ...
    if ".md" in url:
        # return mock readme
        response.json = mock.Mock(return_value={"content": base64.b64encode("mock readme".encode())})
    else:
...
def mock_requests(external_api: bool):
    ...
    with mock.patch("requests.get") as mock_get, mock.patch("sherpa_ai.scrape.extract_github_readme.save_to_pine_cone") :
        mock_get.side_effect = mock_request
        yield

def test_extract_github_readme_with_valid_url_succeeds(mock_requests,):
