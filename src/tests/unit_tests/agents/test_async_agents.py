...
class MockPolicy(BasePolicy):
    def select_action(self, belief):
        return type("obj", (object,), {"action": MockAsyncAction(), "args": {}})
...
async def test_async_run_with_shared_memory():
    ...
    await shared_memory.async_add("task", "test_agent", content="test task")

    agent = MockAgent(belief=Belief(), policy=MockPolicy(), shared_memory=shared_memory)
    result = await agent.async_run()
    assert isinstance(result, TaskResult)
    assert result.status == "success"
    assert result.content == "async_result"
