...
@pytest.mark.parametrize(
    ...
    [
        (
            0,
            "what is unique about Ethiopia calendar?  based on USA calendar association. ",
            (
                """
...
def test_reggration_validator_flow(
    get_llm, test_id, objective, input_data, expected_entities
):
    llm = get_llm(
        __file__, test_reggration_validator_flow.__name__ + f"_{str(test_id)}"
    )
    data = input_data[0]
    ...
    with patch.object(SearchTool, "_run", return_value=data):
        task_agent = QAAgent(
            ...
            num_runs=3,
            validations=[entity_validation, number_validation],
            validation_steps=3,
        )
