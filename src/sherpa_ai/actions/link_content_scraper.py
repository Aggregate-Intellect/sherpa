...
from sherpa_ai.tools import LinkScraperTool, SearchTool


# TODO check for prompt that keep original sentences
...
class LinkContentScraper(BaseRetrievalAction):
    ...
    usage: str = "Get answers from the link content."

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._link_scraper = LinkScraperTool(llm=self.llm)

    def search(self, query) -> list[dict]:
        resources = self._link_scraper._run(
            query,
        )
