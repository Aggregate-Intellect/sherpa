...
class LocalChromaStore(VectorStore):
    """A local Chroma-based vector store, backed directly by chromadb.

    chromadb is already a hard dependency of sherpa-ai (see chroma_vector_store.py),
    so this talks to it directly instead of going through the langchain-chroma
    wrapper package, which brings nothing this class needs.
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        """Embed and add texts to the underlying chromadb collection."""
        texts = list(texts)
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        """Create a LocalChromaStore from a list of texts."""
        store = cls(collection_name=index_name, embedding_function=embedding)
        if texts:
            store.add_texts(texts, metadatas)
        return store
...
def configure_chroma(host: str, port: int, index_name: str, openai_api_key: str):
    ...
    return LocalChromaStore(
        collection_name=cfg.CHROMA_INDEX, embedding_function=embeddings, client=client
    )
