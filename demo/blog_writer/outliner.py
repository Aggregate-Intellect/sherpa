...
class Outliner:
    ...
    def create_blueprint(self, statements, verbose=True):
        ...
        human_template = """Organize the following insights in a logical order. \
                Keep only numbers corresponding to the insight and not the statements of the insights. \
                Cluster points that talk about the same topic. \
                Only include 3-5 insights per topic. \
                Combine each 3-5 topics into a higher level topic. \
                Organize the information in a consistent json structure.

                Desired output format:
                {{
                   [high level topic]: {{
                       [topic]: [list of key insight numbers],
                       [topic]: [list of key insight numbers]\}},
                   [high level topic]: {{
                       [topic]: [list of key insight numbers],
                       [topic]: [list of key insight numbers]}}
